...
export async function executeCode(sourceCode, testCases) {
...
`;
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
                try {
                    response = await axios.post(`${PISTON_API_URL}/execute`, payload);
                    if (response.data && response.data.run) {
                        break; // Success
                    }
                } catch (e) {
                    console.warn(`[PISTON WARNING] Attempt ${attempt} failed:`, e.message);
                    lastError = e;
                    if (attempt < 3) await new Promise(res => setTimeout(res, 500)); // Wait 500ms
                }
            }

            if (!response || !response.data) {
                throw lastError || new Error("Piston execution failed after 3 attempts");
            }
